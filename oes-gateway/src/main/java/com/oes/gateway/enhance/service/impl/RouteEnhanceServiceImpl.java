...
public class RouteEnhanceServiceImpl implements RouteEnhanceService {
  ...
  public Mono<Void> filterBlackList(ServerWebExchange exchange) {
    ...
    try {
      ...
      if (originUri != null) {
        ...
        if (forbid.get()) {
          return HttpUtil.makeWebFluxResponse(response, MediaType.APPLICATION_JSON_VALUE,
              HttpStatus.NOT_ACCEPTABLE, R.fail("黑名单限制，禁止访问"));
        }
      } else {
    ...
    for (Object o : blackList) {
      ...
      if (pathMatcher.match(b.getRequestUri(), uri.getPath()) && BlackList.OPEN
        ...
        if (BlackList.METHOD_ALL.equalsIgnoreCase(b.getRequestMethod())
          ...
          if (StrUtil.isNotBlank(b.getLimitFrom()) && StrUtil.isNotBlank(b.getLimitTo())) {
            if (DateUtil
                .between(LocalTime.parse(b.getLimitFrom()), LocalTime.parse(b.getLimitTo()))) {
              forbid.set(true);
            }
          } else {
            forbid.set(true);
  ...
  private Mono<Void> doRateLimitCheck(AtomicBoolean limit, RateLimitRule rule, URI uri,
    ...
    if (isRateLimitRuleHit) {
      if (StrUtil.isNotBlank(rule.getLimitFrom()) && StrUtil
          .isNotBlank(rule.getLimitTo())) {
        if (DateUtil
            .between(LocalTime.parse(rule.getLimitFrom()), LocalTime.parse(rule.getLimitTo()))) {
          limit.set(true);
        }
      } else {
        limit.set(true);
    ...
    if (limit.get()) {
      ...
      if (count == 0) {
        routeEnhanceCacheService
            .setCurrentRequestCount(requestUri, requestIp, Long.parseLong(rule.getIntervalSec()));
      } else if (count >= Integer.parseInt(rule.getCount())) {
        return HttpUtil.makeWebFluxResponse(response, MediaType.APPLICATION_JSON_VALUE,
            HttpStatus.TOO_MANY_REQUESTS, R.fail("访问频率超限，请稍后再试"));
      } else {
